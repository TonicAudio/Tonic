...
public:
  ...
  DelayTestSynth(){
    ...
    Generator osc = (
                      (
                        (
                          RectWave().freq( freq.smoothed(0.02) * 0.99 ).pwm(0.5f) * aEnv * 0.5
                        )
                      ...
                      (
                        (
                          RectWave().freq(freq.smoothed(0.02) * 1.01).pwm(0.5f) * aEnv * 0.5
                        )
                        >> MonoToStereoPanner().pan(1)
                      )
                    );
    ...
    BasicDelay delay = BasicDelay(0.5f, 1.0f)
      .delayTime( addParameter("delayTime", 0.5f, 0.01f, 1.0f).smoothed(0.5f) )
