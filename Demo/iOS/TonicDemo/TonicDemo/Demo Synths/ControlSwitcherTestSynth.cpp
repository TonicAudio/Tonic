...
public:
  ControlSwitcherTestSynth(){
    ...
    Generator click = SineWave().freq(300 + 1000 * clickEnv ) * clickEnv * 0.25 ;
    ...
    Generator bassEnv = ADSR(0.002, 0.1 ,0, 0.02).doesSustain(false).decay(decay).legato(true).sustain(sustain * sustain).trigger(metro);
    Generator bass =
      SineWave()
      .freq(
        freq
