...
public:
  SimpleStepSeqSynth(){
    ...
    ControlGenerator metro = ControlMetro().bpm(4 * addParameter("tempo", 100, 50, 300));
    
    ControlGenerator step = ControlStepper().end(NUM_STEPS).trigger(metro);
    ControlGenerator transpose = addParameter("transpose", 0, -6, 6);
    ...
    for(int i = 0; i < NUM_STEPS; i++){
      ...
      pitches.addInput(addParameter("step" + to_string(i) + "Pitch", initialValue, 10, 80, false ));
      cutoffs.addInput(addParameter("step" + to_string(i) + "Cutoff", 500, 30, 1500, false));
    }
