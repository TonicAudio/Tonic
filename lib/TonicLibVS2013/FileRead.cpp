...
namespace Tonic {
	...
	bool FileRead::getRawInfo(const char *fileName, unsigned int nChannels, FileReadFormat format, TonicFloat rate)
	{
		...
		if (stat(fileName, &filestat) == -1) {
			Tonic::error("FileRead: Could not stat RAW file (" + fileName + ").");
			return false;
		}
		if (nChannels == 0) {
			Tonic::error("FileRead: number of channels can't be 0 (" + fileName + ").");
			return false;
		}
		...
		if (format == FILEREAD_SINT8) sampleBytes = 1;
			...
			oStream_ << "FileRead: FileReadFormat " << format << " is invalid (" << fileName << ").";
			return false;
		}
